...
@Component({
  selector: 'app-entrega-clientes',
  templateUrl: './entrega-clientes.component.html',
  styleUrls: ['./entrega-clientes.component.css']
})
export class EntregaClientesComponent implements OnInit {
  preguntas:any [] = [];
  empresas:any [] = [];
  ...
  ngOnInit(): void {
    this.getEntregaClientMes();
    this.getEntregaClientRuta();
  }
  changeChart() {
  ...
  getEntregaClientMes(){
    this.jsonService.getEntregaClientMes().subscribe((resp: any) => {
      this.preguntas = resp.ProcesoEntregaClientesMes.map((resp: any) => {
          return {
            abr: Number(resp.Abr),
            ago: Number(resp.Ago),
            dic: Number(resp.Dic),
  ...
  getEntregaClientRuta() {
    this.jsonService.getEntregaClientRuta(this.fechaInicio, this.fechaFin).subscribe((resp: any) => {
      this.proveedores = [...new Set(resp.ProcesoEntregaClientesRuta.map((e:any) => e.empresa))]
        this.bks = [...new Set(this.proveedores.map(b => b.bk))];
        this.empresas = resp.ProcesoEntregaClientesRuta.map((res: any) => {
          console.log()
          return {
            abr: Number(res.Abr),
            ago: Number(res.Ago),
